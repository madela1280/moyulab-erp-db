...
import type { PartnerAllCell, PartnerAllRow } from "../types.partnerAll";
...
export function buildGrandTotal(args: {
  ...
  for (const r of rows) {
    if (r.rowType === "grandTotal") continue;
    for (const p of periods) addCell(values[p.key], r.values?.[p.key]);
  }

  return {
    rowType: "grandTotal" as const,
    section: "합계" as const,
    label: "합계",
    values,
