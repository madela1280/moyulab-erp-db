...
function ToolButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="inline-flex items-center gap-1.5 text-xs px-2 py-1 bg-slate-50 border border-slate-200 rounded text-slate-700 hover:bg-slate-100 active:bg-slate-200"
    >
      <Icon name={icon} />
      <span>{children}</span>
    </button>
...
export default function GridHeader({ onAdd10 }: Props) {
  return (
    <div className="w-full flex items-center gap-2 px-2 py-2 bg-white border-b">
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        <ToolButton icon="swap">열이동</ToolButton>
      </div>
    </div>
  );
}
