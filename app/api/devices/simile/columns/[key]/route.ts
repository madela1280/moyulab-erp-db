...
async function ensureColumnsTable() {
  await query(`
    CREATE TABLE IF NOT EXISTS device_simile_columns (
      id   INT PRIMARY KEY,
  ...
  await query(`
    INSERT INTO device_simile_columns (id, data)
    VALUES (1, '{}'::jsonb)
...
/**
 * base(=simileColumns)는 항상 base 순서 고정.
...
async function loadOrder() {
  ...
  const baseOrder = [...(simileColumns as unknown as string[])];

  const r = await query(`SELECT data FROM device_simile_columns WHERE id=1 LIMIT 1`);
...
/**
 * DELETE /api/devices/simile/columns/:key
 * - base 컬럼(simileColumns)은 삭제 금지
 */
export async function DELETE(req: Request) {
  try {
    ...
    const baseSet = new Set<string>([...(simileColumns as unknown as string[])]);
    if (baseSet.has(key)) {
    ...
    console.error("DELETE /api/devices/simile/columns/[key] error:", e);
