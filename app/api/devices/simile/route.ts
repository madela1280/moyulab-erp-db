...
export async function GET(req: Request) {
  try {
    ...
    if ((sp.get("meta") || "").toLowerCase() === "count") {
      const r = await query(`SELECT COUNT(*)::int AS count FROM device_simile_order`);
    ...
    if (idsParam) {
      ...
      const r = await query(
        ...
        FROM device_simile s
        JOIN device_simile_order o ON o.simile_id = s.id
    ...
    const totalR = await query(`SELECT COUNT(*)::int AS total FROM device_simile_order`);
    ...
    console.error("GET /api/devices/simile error:", e);
...
export async function POST(req: Request) {
  try {
    ...
    const r = await query(`INSERT INTO device_simile (data) VALUES ($1) RETURNING id, data`, [
      body ?? {},
    ]);
    const created = r.rows[0];

    const maxR = await query(`SELECT COALESCE(MAX(sort_key), 0) AS max FROM device_simile_order`);
    ...
    await query(
      ...
      INSERT INTO device_simile_order (simile_id, sort_key)
      VALUES ($1, $2)
      ON CONFLICT (simile_id) DO NOTHING
      `,
      [created.id, nextKey]
    );
    ...
    console.error("POST /api/devices/simile error:", e);
    return NextResponse.json({ error: "SERVER" }, { status: 500 });
  }
}