...
// ✅ 스윙맥시 테이블이 아직 DB에 없어서 500이 나는 경우를 막기 위해,
//    API 호출 시 안전하게 "필요 테이블을 자동 생성" (IF NOT EXISTS) + order 누락 보정
// ✅ (Perf) 요청마다 실행하지 않고, 같은 Node 프로세스에서는 1회만 실행
let _ensureSwingMaxiTablesPromise: Promise<void> | null = null;

async function ensureSwingMaxiTables() {
  if (_ensureSwingMaxiTablesPromise) return _ensureSwingMaxiTablesPromise;

  _ensureSwingMaxiTablesPromise = (async () => {
    await query(`
    ...
    _ensureSwingMaxiTablesPromise = null;
    throw e;
  });

  return _ensureSwingMaxiTablesPromise;
}
