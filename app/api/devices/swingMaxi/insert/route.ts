...
/**
 * POST /api/devices/swingMaxi/insert
...
export async function POST(req: Request) {
  try {
    ...
    if (beforeId) {
      const r = await query(
        `SELECT sort_key::numeric AS sort_key FROM device_swing_maxi_order WHERE swing_maxi_id=$1`,
        [beforeId]
      );
    ...
    if (beforeKey == null && afterKey == null) {
      const maxR = await query(
        `SELECT COALESCE(MAX(sort_key), 0)::numeric AS max FROM device_swing_maxi_order`
      );
      ...
      try {
        for (let i = 0; i < count; i++) {
          const created = await query(
            `INSERT INTO device_swing_maxi (data) VALUES ($1) RETURNING id`,
            [{}]
          );
          ...
          await query(
            `INSERT INTO device_swing_maxi_order (swing_maxi_id, sort_key) VALUES ($1, $2)`,
            [id, sortKey]
          );
    ...
    console.error("POST /api/devices/swingMaxi/insert error:", e);
