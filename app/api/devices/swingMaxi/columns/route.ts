...
/**
 * ✅ 핵심 정책
 * - base(=swingMaxiColumns)는 항상 base 순서를 유지한다.
...
export async function GET() {
  try {
    ...
    console.error("GET /api/devices/swingMaxi/columns error:", e);
...
export async function POST(req: Request) {
  try {
    ...
    console.error("POST /api/devices/swingMaxi/columns error:", e);
    return NextResponse.json({ ok: false, error: "SERVER" }, { status: 500 });
  }
}