...
async function existsSwingMaxiBySystemNo(systemNoRaw: any) {
  const systemNo = n(systemNoRaw).toLowerCase();
...
/**
 * POST /api/devices/swingMaxi/bulk-delete
 * body: { ids: number[] }
 */
export async function POST(req: Request) {
  try {
    ...
    const r = await query(
      `DELETE FROM device_swing_maxi WHERE id = ANY($1::int[]) RETURNING id`,
      [ids]
    );

    for (const systemNo of deletedSystemNos) {
      const stillExists = await existsSwingMaxiBySystemNo(systemNo);
      if (!stillExists) {
    ...
    console.error("POST /api/devices/swingMaxi/bulk-delete error:", e);
