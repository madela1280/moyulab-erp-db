...
/**
 * 스윙맥시 그리드 설정 저장/로드 (브라우저 저장소 금지 → DB 저장)
 * - 단일 row(id=1)로 관리
 *
 * GET  /api/devices/swingMaxi/grid-settings  -> { columnOrder, colWidthUnitByKey }
 * POST /api/devices/swingMaxi/grid-settings  -> upsert
 */

async function ensureSettingsTable() {
  await query(`
    CREATE TABLE IF NOT EXISTS device_swing_maxi_grid_settings (
      id   INT PRIMARY KEY,
  ...
  await query(`
    INSERT INTO device_swing_maxi_grid_settings (id, data)
    VALUES (1, '{}'::jsonb)
...
export async function GET() {
  try {
    ...
    const r = await query(
      `SELECT data FROM device_swing_maxi_grid_settings WHERE id=1 LIMIT 1`
    );
    ...
    console.error("GET /api/devices/swingMaxi/grid-settings error:", e);
...
export async function POST(req: Request) {
  try {
    ...
    console.error("POST /api/devices/swingMaxi/grid-settings error:", e);
    return NextResponse.json({ error: "SERVER" }, { status: 500 });
  }
}