...
export async function GET(req: Request) {
  try {
    ...
    if ((sp.get("meta") || "").toLowerCase() === "count") {
      const r = await query(`SELECT COUNT(*)::int AS count FROM device_gaksimil_order`);
    ...
    const totalR = await query(`SELECT COUNT(*)::int AS total FROM device_gaksimil_order`);
    ...
    console.error("GET /api/devices/gaksiMil error:", e);
...
export async function POST(req: Request) {
  try {
    ...
    const r = await query(`INSERT INTO device_gaksimil (data) VALUES ($1) RETURNING id, data`, [
      body ?? {},
    ]);
    const created = r.rows[0];

    const maxR = await query(`SELECT COALESCE(MAX(sort_key), 0) AS max FROM device_gaksimil_order`);
    ...
    await query(
      ...
      INSERT INTO device_gaksimil_order (gaksimil_id, sort_key)
      VALUES ($1, $2)
      ON CONFLICT (gaksimil_id) DO NOTHING
      `,
      [created.id, nextKey]
    );
    ...
    console.error("POST /api/devices/gaksiMil error:", e);
    return NextResponse.json({ error: "SERVER" }, { status: 500 });
  }
}