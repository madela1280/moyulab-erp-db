...
/**
 * POST /api/devices/gaksiMil/insert
...
export async function POST(req: Request) {
  try {
    ...
    if (beforeId) {
      const r = await query(
        `SELECT sort_key::numeric AS sort_key FROM device_gaksimil_order WHERE gaksimil_id=$1`,
        [beforeId]
      );
    ...
    if (beforeKey == null && afterKey == null) {
      const maxR = await query(
        `SELECT COALESCE(MAX(sort_key), 0)::numeric AS max FROM device_gaksimil_order`
      );
      ...
      try {
        for (let i = 0; i < count; i++) {
          const created = await query(`INSERT INTO device_gaksimil (data) VALUES ($1) RETURNING id`, [
            {},
          ]);
          const id = Number(created.rows[0]?.id);
          const sortKey = max + (i + 1) * 1000;

          await query(`INSERT INTO device_gaksimil_order (gaksimil_id, sort_key) VALUES ($1, $2)`, [
            id,
            sortKey,
          ]);
    ...
    console.error("POST /api/devices/gaksiMil/insert error:", e);
