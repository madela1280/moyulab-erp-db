...
/**
 * POST /api/devices/swing/bulk-delete
 * body: { ids: number[] }
 *
 * - 여러 행 삭제를 1쿼리로 처리
 */
export async function POST(req: Request) {
  try {
    ...
    const r = await query(`DELETE FROM device_swing WHERE id = ANY($1::int[]) RETURNING id`, [
      ids,
    ]);
    ...
    console.error("POST /api/devices/swing/bulk-delete error:", e);
