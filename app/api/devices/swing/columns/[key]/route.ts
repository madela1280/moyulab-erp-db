...
async function ensureColumnsTable() {
  await query(`
    CREATE TABLE IF NOT EXISTS device_swing_columns (
      id   INT PRIMARY KEY,
  ...
  await query(`
    INSERT INTO device_swing_columns (id, data)
    VALUES (1, '{}'::jsonb)
...
/**
 * base(=swingColumns)는 항상 base 순서 고정.
...
async function loadOrder() {
  ...
  const baseOrder = [...(swingColumns as unknown as string[])];

  const r = await query(`SELECT data FROM device_swing_columns WHERE id=1 LIMIT 1`);
...
/**
 * DELETE /api/devices/swing/columns/:key
 * - base 컬럼(swingColumns)은 삭제 금지
 */
export async function DELETE(req: Request) {
  try {
    ...
    const baseSet = new Set<string>([...(swingColumns as unknown as string[])]);
    if (baseSet.has(key)) {
    ...
    console.error("DELETE /api/devices/swing/columns/[key] error:", e);
