...
async function ensureColumnsTable() {
  await query(`
    CREATE TABLE IF NOT EXISTS device_lactina_columns (
      id   INT PRIMARY KEY,
  ...
  await query(`
    INSERT INTO device_lactina_columns (id, data)
    VALUES (1, '{}'::jsonb)
...
/**
 * base(=lactinaColumns)는 항상 base 순서 고정.
...
async function loadOrder() {
  ...
  const baseOrder = [...(lactinaColumns as unknown as string[])];

  const r = await query(`SELECT data FROM device_lactina_columns WHERE id=1 LIMIT 1`);
...
/**
 * DELETE /api/devices/lactina/columns/:key
 * - base 컬럼(lactinaColumns)은 삭제 금지
 */
export async function DELETE(req: Request) {
  try {
    ...
    const baseSet = new Set<string>([...(lactinaColumns as unknown as string[])]);
    if (baseSet.has(key)) {
    ...
    console.error("DELETE /api/devices/lactina/columns/[key] error:", e);
