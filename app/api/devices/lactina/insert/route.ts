...
/**
 * POST /api/devices/lactina/insert
...
export async function POST(req: Request) {
  try {
    ...
    if (beforeId) {
      const r = await query(
        `SELECT sort_key::numeric AS sort_key FROM device_lactina_order WHERE lactina_id=$1`,
        [beforeId]
      );
    ...
    if (beforeKey == null && afterKey == null) {
      const maxR = await query(
        `SELECT COALESCE(MAX(sort_key), 0)::numeric AS max FROM device_lactina_order`
      );
      ...
      try {
        for (let i = 0; i < count; i++) {
          const created = await query(
            `INSERT INTO device_lactina (data) VALUES ($1) RETURNING id`,
            [{}]
          );
          ...
          await query(
            `INSERT INTO device_lactina_order (lactina_id, sort_key) VALUES ($1, $2)`,
            [id, sortKey]
          );
    ...
    console.error("POST /api/devices/lactina/insert error:", e);
