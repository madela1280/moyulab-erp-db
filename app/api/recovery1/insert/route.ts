...
export async function POST(req: Request) {
  ...
  // 존재 체크
  if (beforeId !== null) {
    const r = await query(`SELECT 1 FROM recovery1_order WHERE recovery1_id=$1 LIMIT 1`, [beforeId]);
    if (!r.rows.length) return NextResponse.json({ error: "BEFORE_NOT_FOUND" }, { status: 404 });
  }
  if (afterId !== null) {
    const r = await query(`SELECT 1 FROM recovery1_order WHERE recovery1_id=$1 LIMIT 1`, [afterId]);
