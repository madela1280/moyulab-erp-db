...
export async function GET(req: Request) {
  try {
    ...
    if ((sp.get("meta") || "").toLowerCase() === "count") {
      const r = await query(`SELECT COUNT(*)::int AS count FROM recovery2_order`);
    ...
    if (idsParam) {
      ...
      const r = await query(
        ...
        FROM recovery2 r
        JOIN recovery2_order o ON o.recovery2_id = r.id
    ...
    const totalR = await query(`SELECT COUNT(*)::int AS total FROM recovery2_order`);
    ...
    if (tailData) {
      const cursorR = await query(
        ...
        WITH last_data AS (
          ...
          FROM recovery2_order o
          JOIN recovery2 r ON r.id = o.recovery2_id
          WHERE EXISTS (
            SELECT 1
        ...
        last_any AS (
          SELECT o.sort_key, o.recovery2_id AS id
          FROM recovery2_order o
          ORDER BY o.sort_key DESC, o.recovery2_id DESC
          LIMIT 1
        )
        SELECT
      ...
      const posR = await query(
        ...
        FROM recovery2_order o
        WHERE (o.sort_key, o.recovery2_id) <= ($1::numeric, $2::int)
        `,
          ...
          SELECT
            CASE
              ...
              ELSE (
                SELECT COUNT(*)::int
                FROM recovery2_order o
                WHERE (o.sort_key, o.recovery2_id) < ((SELECT sort_key FROM first_row), (SELECT id FROM first_row))
              ) + 1
            END AS base_index
        )
        SELECT
    ...
    console.error("GET /api/recovery2 error:", e);
...
export async function POST(req: Request) {
  try {
    ...
    const r = await query(`INSERT INTO recovery2 (data) VALUES ($1) RETURNING id, data`, [data]);
    const created = r.rows[0];

    const maxR = await query(`SELECT COALESCE(MAX(sort_key), 0) AS max FROM recovery2_order`);
    ...
    await query(
      ...
      INSERT INTO recovery2_order (recovery2_id, sort_key)
      VALUES ($1, $2)
      ON CONFLICT (recovery2_id) DO NOTHING
      `,
      [created.id, nextKey]
    );
    ...
    console.error("POST /api/recovery2 error:", e);
    return NextResponse.json({ error: "SERVER" }, { status: 500 });
  }
}