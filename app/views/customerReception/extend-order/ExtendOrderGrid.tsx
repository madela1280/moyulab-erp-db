...
// app/views/customerReception/extend-order/ExtendOrderGrid.tsx
//
// 연장·연체료 그리드. packaging-order/PackagingOrderGrid.tsx와 같은 동작
// (열 이동/너비 조절, 영역지정 복사(Ctrl+C), 영역지정 삭제(Delete))을 제공하되
// 포장재구매 코드는 건드리지 않기 위해 이 기능 전용 파일로 분리했다.
//
// 맨 앞 "확인" 체크박스 컬럼은 열 이동/너비 대상이 아닌 고정 컬럼이라
// EXTEND_ORDER_COLUMNS 목록과 별도로 그린다.
...
import {
  buildExtendOrderCellRange,
  isExtendOrderCellInRange,
  makeExtendOrderTSV,
  type ExtendOrderCellPoint,
  type ExtendOrderCellRange,
} from "@/views/customerReception/extend-order/clipboard";

export type ExtendOrderSortMode = "none" | "confirmed-first" | "waiting-first";

type ExtendOrderGridProps = {
  rows?: ExtendOrderRow[];
  columns?: ExtendOrderColumn[];
  isColumnEditMode?: boolean;
  onRowsChange?: (rows: ExtendOrderRow[]) => void;
  onColumnsChange?: (columns: ExtendOrderColumn[]) => void;
  selectedIds?: Set<string>;
  onToggleSelect?: (id: string) => void;
  onToggleSelectAll?: (checked: boolean) => void;
  sortMode?: ExtendOrderSortMode;
  onToggleSort?: () => void;
};

function isMultiCellRange(range: ExtendOrderCellRange | null) {
...
function isReadonlyColumn(col: ExtendOrderColumn) {
  return (
    col.type === "status" || col.type === "datetime" || col.type === "settlementType" || col.type === "readonly"
  );
}
...
export default function ExtendOrderGrid({
  ...
  sortMode = "none",
  onToggleSort,
}: ExtendOrderGridProps) {
  const gridRef = useRef<HTMLDivElement | null>(null);

  const displayRows = useMemo(() => {
    if (Array.isArray(rows) && rows.length > 0) return rows;
    return Array.from({ length: 10 }, (_, index) => createEmptyExtendOrderRow(index + 1));
  }, [rows]);

  const displayColumns = useMemo(() => {
    const baseColumns = Array.isArray(columns) && columns.length > 0 ? columns : EXTEND_ORDER_COLUMNS;
    return baseColumns.map((col) => ({ ...col, width: normalizeWidth(col.width) }));
  }, [columns]);

  const [selectionAnchor, setSelectionAnchor] = useState<ExtendOrderCellPoint | null>(null);
  const [selectedRange, setSelectedRange] = useState<ExtendOrderCellRange | null>(null);
  ...
  function focusCell(rowIndex: number, colIndex: number) {
    window.setTimeout(() => {
      const input = gridRef.current?.querySelector<HTMLInputElement>(
        `input[data-eo-row="${rowIndex}"][data-eo-col="${colIndex}"]`
      );
      if (!input) return;
      input.focus();
  ...
  function selectSingleCell(rowIndex: number, colIndex: number) {
    ...
    setSelectedRange(buildExtendOrderCellRange(point, point));
  }
  ...
  function handleCellMouseEnter(rowIndex: number, colIndex: number, buttons: number) {
    ...
    setSelectedRange(buildExtendOrderCellRange(selectionAnchor, { rowIndex, colIndex }));
  }
  ...
  function handleColumnWidthChange(col: ExtendOrderColumn, value: string) {
  ...
  function handleCopy(e: ClipboardEvent<HTMLDivElement>) {
    if (!selectedRange) return;
    const tsv = makeExtendOrderTSV(displayRows, displayColumns, selectedRange);
    if (!tsv) return;
    e.preventDefault();
          ...
          {displayRows.map((row, rowIndex) => (
            <tr key={row.id} className="h-8">
              ...
              {displayColumns.map((col, colIndex) => {
                const selected = isExtendOrderCellInRange(rowIndex, colIndex, selectedRange);
                ...
                if (col.type === "datetime" || col.type === "settlementType" || col.type === "readonly") {
                ...
                return (
                  ...
                  >
                    <input
                      data-eo-row={rowIndex}
                      data-eo-col={colIndex}
