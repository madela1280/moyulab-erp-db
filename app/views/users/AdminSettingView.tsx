...
export default function AdminSettingView() {
  ...
  return (
    <div className="px-4 py-3 text-sm text-gray-700 max-w-xl">
      <div className="font-semibold mb-3">관리자(회사 마스터) 설정</div>

      {loadingAdmin || !admin ? (
        <div className="text-xs text-gray-500">관리자 정보를 불러오는 중...</div>
      ) : (
        <>
          <div className="mb-3">
            <label className="block text-xs mb-1">관리자 아이디</label>
            <div className="px-2 py-1 border rounded bg-gray-100 text-xs">
              {admin.username}
            </div>
          </div>
