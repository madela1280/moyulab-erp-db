// app/views/sms/components/SmsSendPanel.tsx
"use client";
...
export default function SmsSendPanel(props: {
  ...
  const { subCategory, baseDate, selectedCount, selectedRows } = props;
  ...
  async function runSend(scope: "all" | "selected", dryRun: boolean) {
    ...
    try {
      ...
      setLastMsg(
        res.ok
          ? `발송 요청 완료: requested=${res.requestedCount ?? 0}, failed=${res.failedCount ?? 0}`
          : `발송 요청 실패: ${res.error ?? "unknown"}`
      );
  ...
  async function runSync() {
    ...
    try {
      ...
      setLastMsg(`결과 동기화: success=${res.success}, fail=${res.fail}, processing=${res.processing}`);
    } catch (e: any) {
  ...
  return (
    <div className="p-3 flex flex-col gap-3">
      ...
      <div className="text-xs text-gray-600">
        소카테고리: <span className="font-semibold">{subCategory}</span>
        <br />
        기준일: <span className="font-mono">{baseDate || "-"}</span>
      </div>

      <div className="border rounded p-2 bg-gray-50">
        <div className="text-xs font-semibold text-gray-700 mb-1">미리보기(샘플)</div>
        {sample ? (
          <div className="text-[11px] text-gray-700 leading-5">
            안내분류: <span className="font-semibold">{sample.안내분류 || "-"}</span>
            <br />
            수취인명: <span className="font-semibold">{sample.수취인명 || "-"}</span>
            <br />
            연락처: <span className="font-mono">{sample.연락처1 || "-"}</span>
            <br />
            종료일: <span className="font-mono">{sample.종료일 || "-"}</span>
            <br />
            만기표시: <span className="font-mono">{sample.만기표시 || "-"}</span>
          </div>
        ) : (
      ...
      <div className="flex flex-col gap-2">
        <button
          ...
          disabled={busy}
          title="실제 발송 없이 서버 검증만(서버 구현 필요)"
        >
          전체 검증(dry-run)
        </button>
