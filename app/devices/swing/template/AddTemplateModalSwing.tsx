...
export default function AddTemplateModalSwing({ open, onClose, onChanged }: Props) {
  ...
  const baseSet = useMemo(() => new Set<string>([...(swingColumns as unknown as string[])]), []);
  ...
  async function loadColumns() {
    ...
    setError("");
    try {
      const r = await fetch("/api/devices/swing/columns", { cache: "no-store" });
  ...
  async function handleAdd() {
    ...
    setError("");
    try {
      const r = await fetch("/api/devices/swing/columns", {
        method: "POST",
  ...
  async function handleDelete(key: string) {
    ...
    setError("");
    try {
      const r = await fetch(`/api/devices/swing/columns/${encodeURIComponent(k)}`, {
        method: "DELETE",
      });
    ...
    >
      ...
      >
        <div className="px-4 py-3 border-b flex items-center justify-between">
          <div className="font-semibold text-slate-800">스윙 양식(컬럼) 추가/삭제</div>
          <button
          ...
          <div className="border rounded p-3">
            ...
            <div className="flex flex-col gap-2">
              ...
              <select
                ...
                disabled={loading}
              >
                {(order.length ? order : [...(swingColumns as unknown as string[])]).map((k) => (
          ...
          <div className="border rounded p-3">
            ...
            <div className="max-h-[260px] overflow-auto border rounded">
              {customColumns.map((k) => (
                <div
                  key={k}
                  className="flex items-center justify-between px-2 py-2 border-b last:border-b-0"
                >
