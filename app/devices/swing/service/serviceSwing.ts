...
export async function listSwingRows(params?: { limit?: number; tailData?: 1; ids?: number[] }) {
  ...
  const url = qs ? `/api/devices/swing?${qs}` : `/api/devices/swing`;

  return fetchJson<
    SwingRow[] | { rows: SwingRow[]; total?: number; baseIndex?: number; count?: number }
  >(url, { cache: "no-store" });
}

export async function getSwingRow(id: number) {
  return fetchJson<SwingRow>(`/api/devices/swing/${id}`, { cache: "no-store" });
}
 ...
 */
export async function patchSwingRow(id: number, patch: Record<string, any>) {
  const res = await fetchJson<SwingRow>(`/api/devices/swing/${id}`, {
    method: "PATCH",
...
export async function createSwingRow(data: Record<string, any>) {
  const res = await fetchJson<SwingRow>(`/api/devices/swing`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function bulkPatchSwing(args: BulkPatchArgs) {
  const res = await fetchJson<{ ok: true; updatedCount: number; updatedIds: number[] }>(
    `/api/devices/swing/bulk-patch`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(args),
...
export async function bulkDeleteSwing(args: BulkDeleteArgs) {
  const res = await fetchJson<{ ok: true; deletedCount: number; deletedIds: number[] }>(
    `/api/devices/swing/bulk-delete`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(args),
...
export async function exportSwingCsv(body: { filter?: any }) {
  const payload = {
    filter: normalizeFilterForExport(body?.filter),
  };

  const r = await fetch(`/api/devices/swing/export`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
