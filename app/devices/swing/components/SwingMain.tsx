...
import SwingHeader from "@/devices/swing/components/SwingHeader";
import SwingGrid, { SwingGridHandle } from "@/devices/swing/components/SwingGrid";
import { useSwingColumnConfig } from "@/devices/swing/column-config/useSwingColumnConfig";
import { exportSwingCsv, insertSwingRows } from "@/devices/swing/service/serviceSwing";

import AddTemplateModalSwing from "@/devices/swing/template/AddTemplateModalSwing";

import ColorPopover, { type SwingSoftColor } from "@/devices/swing/color/ColorPopover";
...
import { defaultSortState, type SwingSortState } from "@/devices/swing/filter/useSwingSort";

import { syncListen } from "@/global-sync/sync-engine";

export default function SwingMain() {
  const gridRef = useRef<SwingGridHandle | null>(null);
  ...
  const [sortState, setSortState] = useState<SwingSortState>(() => defaultSortState());
  ...
  async function handleAdd10() {
    await insertSwingRows({ count: 10, beforeId: null, afterId: null });
  ...
  async function handleDownload() {
    const blob = await exportSwingCsv({ filter: { filterState, sortState } });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "swing.csv";
  ...
  async function applyColor(color: SwingSoftColor, mode: ColorApplyMode) {
