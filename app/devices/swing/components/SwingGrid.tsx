...
const SwingGrid = forwardRef<SwingGridHandle, Props>(function SwingGrid(props, ref) {
  ...
  function getActiveSwingRowId(): number | null {
    try {
  ...
  async function handleFocus(rowId: number, key: string, initialValue: string, e: any) {
    ...
    const pending = existingPending ?? acquireLock("swing", rowId);
    ...
    if (result?.ok) {
      ...
      await releaseLock("swing", rowId).catch(() => {});
      return;
    }
  ...
  async function ensureSwingRowLock(rowId: number) {
    ...
    const retry = await acquireLock("swing", rowId).catch(() => null);
    if (retry?.ok) {
      myRowLocksRef.current[rowId] = true;
      setMyRowLocks((prev) => ({ ...prev, [rowId]: true }));
      return true;
  ...
  async function clearSingleSelectedCell() {
    ...
    const hasLock = await ensureSwingRowLock(row.id);
                      ...
                      >
                       <input
                          ...
                          onBlur={async (e) => {
                            ...
                            if (!hasLock) {
                              const retry = await acquireLock("swing", blurRowId).catch(() => null);
                              if (retry?.ok) {
                                hasLock = true;
                                myRowLocksRef.current[blurRowId] = true;
                                setMyRowLocks((prev) => ({ ...prev, [blurRowId]: true }));
                            ...
                            try {
                              ...
                              const nextFocusedRowId = getActiveSwingRowId();
                              ...
                              if (!keepRowLock && hasLock) {
                                await releaseLock("swing", blurRowId).catch(() => {});
