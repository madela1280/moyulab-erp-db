...
export async function listGaksiMilRows(params?: {
  limit?: number;
  tailData?: 1;
  ids?: number[];
}) {
  ...
  const url = qs ? `/api/devices/gaksiMil?${qs}` : `/api/devices/gaksiMil`;
...
export async function getGaksiMilRow(id: number) {
  return fetchJson<GaksiMilRow>(`/api/devices/gaksiMil/${id}`, { cache: "no-store" });
}
 ...
 */
export async function patchGaksiMilRow(id: number, patch: Record<string, any>) {
  const res = await fetchJson<GaksiMilRow>(`/api/devices/gaksiMil/${id}`, {
    method: "PATCH",
...
export async function createGaksiMilRow(data: Record<string, any>) {
  const res = await fetchJson<GaksiMilRow>(`/api/devices/gaksiMil`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function insertGaksiMilRows(args: InsertArgs) {
  const res = await fetchJson<{ ok: true; insertedRows: Array<{ id: number; sort_key?: number }> }>(
    `/api/devices/gaksiMil/insert`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function bulkPatchGaksiMil(args: BulkPatchArgs) {
  const res = await fetchJson<{ ok: true; updatedCount: number; updatedIds: number[] }>(
    `/api/devices/gaksiMil/bulk-patch`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(args),
...
export async function bulkDeleteGaksiMil(args: BulkDeleteArgs) {
  const res = await fetchJson<{ ok: true; deletedCount: number; deletedIds: number[] }>(
    `/api/devices/gaksiMil/bulk-delete`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(args),
...
export async function exportGaksiMilCsv(body: { filter?: any }) {
  const payload = {
    filter: normalizeFilterForExport(body?.filter),
  };

  const r = await fetch(`/api/devices/gaksiMil/export`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
