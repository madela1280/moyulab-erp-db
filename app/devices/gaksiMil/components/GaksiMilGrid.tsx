...
const GaksiMilGrid = forwardRef<GaksiMilGridHandle, Props>(function GaksiMilGrid(props, ref) {
  const { rows, setRows, setTotalCount, baseIndex, loading, error, reload } = useGaksiMilRows();
  ...
  async function handleFocus(rowId: number, key: string, initialValue: string, e: any) {
    ...
    const result = await acquireLock("gaksimil", rowId);

    const stillActive = editingCellRef.current?.rowId === rowId && editingCellRef.current?.key === key;

    if (!stillActive) {
      if (result.ok) await releaseLock("gaksimil", rowId);
      return;
    }
                      ...
                      >
                        <input
                          ...
                          onBlur={async (e) => {
                            ...
                            await releaseLock("gaksimil", row.id);
