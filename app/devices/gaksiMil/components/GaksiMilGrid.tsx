...
import { useGaksiMilRows } from "@/devices/gaksiMil/hooks/useGaksiMilRows";
import {
  bulkDeleteGaksiMil,
  bulkPatchGaksiMil,
  insertGaksiMilRows,
  patchGaksiMilRow,
  type GaksiMilRow,
} from "@/devices/gaksiMil/service/serviceGaksiMil";

import ColumnFilterPopover from "@/devices/gaksiMil/filter/ColumnFilterPopover";
import {
  applyGaksiMilFilter,
...
import {
  applyGaksiMilSort,
  type GaksiMilSortState,
} from "@/devices/gaksiMil/filter/useGaksiMilSort";

import { useUnifiedRentalStatus } from "@/devices/gaksiMil/derived/useUnifiedRentalStatus";

import { buildColorBulkPatch } from "@/devices/gaksiMil/color/applyGaksiMilColor";
...
export type GaksiMilGridHandle = {
  reload: (options?: { silent?: boolean }) => Promise<void>;
  applyColorToSelection: (color: GaksiMilSoftColor, mode: "text" | "cell") => Promise<void>;
};
...
type Props = {
  ...
  sortState: GaksiMilSortState;
  onSortStateChange: (next: GaksiMilSortState) => void;
};
...
const GaksiMilGrid = forwardRef<GaksiMilGridHandle, Props>(function GaksiMilGrid(props, ref) {
  const { rows, setRows, setTotalCount, loading, error, reload } = useGaksiMilRows();

  const { rentingDeviceNoSet, rentingInfoByDeviceNo } = useUnifiedRentalStatus();

  const isColumnEditMode = !!props.isColumnEditMode;

  const [columnOrderState, setColumnOrderState] = useState<string[]>(() => [...gaksiMilColumns]);
  ...
  const displayRows = useMemo(() => {
    const filtered = applyGaksiMilFilter(rows as any, props.filterState);
    const sorted = applyGaksiMilSort(filtered as any, props.sortState);
    return sorted as GaksiMilRow[];
  ...
  function getSelectedRowSlice() {
    if (!selectedRowRange) return { start: 0, end: -1, slice: [] as GaksiMilRow[] };
  ...
  function getActiveGaksiMilRowId(): number | null {
    try {
  ...
  async function handleFocus(rowId: number, key: string, initialValue: string, e: any) {
    ...
    const pending = existingPending ?? acquireLock("gaksimil", rowId);
    ...
    if (result?.ok) {
      ...
      await releaseLock("gaksimil", rowId).catch(() => {});
      return;
    }
  ...
  async function saveCell(rowId: number, key: string, value: string) {
    ...
    await patchGaksiMilRow(rowId, { [key]: v });
  }

  async function ensureGaksiMilRowLock(rowId: number) {
    ...
    const retry = await acquireLock("gaksimil", rowId).catch(() => null);
    if (retry?.ok) {
      myRowLocksRef.current[rowId] = true;
      setMyRowLocks((prev) => ({ ...prev, [rowId]: true }));
      return true;
    ...
    const hasLock = await ensureGaksiMilRowLock(row.id);
  ...
  function getDisplayValue(row: GaksiMilRow, colKey: string) {
    ...
    if (colKey === "원가") return formatWon(row.data?.[colKey]);
  ...
  const pasteCatcherRef = useRef<HTMLTextAreaElement | null>(null);
   ...
   async function clearSelection() {
    ...
    if (selectedCellRange) {
      ...
      setRows(local);
      if (updates.length) await bulkPatchGaksiMil({ updates });
      setContextMenu(null);
      return;
    }
    ...
    await bulkPatchGaksiMil({ updates });
    setContextMenu(null);
  }
  ...
  async function handleInsertRows() {
    ...
    await insertGaksiMilRows({ count, beforeId, afterId });
    await reload();
  ...
  async function applyColorToSelection(color: GaksiMilSoftColor, mode: "text" | "cell") {
    if (!selectedCellRange) return;
                      ...
                      >
                        <input
                          ...
                          onBlur={async (e) => {
                            ...
                            if (!hasLock) {
                              const retry = await acquireLock("gaksimil", blurRowId).catch(() => null);
                              if (retry?.ok) {
                                hasLock = true;
                                myRowLocksRef.current[blurRowId] = true;
                                setMyRowLocks((prev) => ({ ...prev, [blurRowId]: true }));
                            ...
                            try {
                              ...
                              const nextFocusedRowId = getActiveGaksiMilRowId();
                              ...
                              if (!keepRowLock && hasLock) {
                                await releaseLock("gaksimil", blurRowId).catch(() => {});
