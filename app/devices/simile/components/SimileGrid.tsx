...
import { simileColumns } from "@/devices/simile/columns/simileColumns";
import { useSimileRows } from "@/devices/simile/hooks/useSimileRows";
import {
  bulkDeleteSimile,
  bulkPatchSimile,
  insertSimileRows,
  patchSimileRow,
  type SimileRow,
} from "@/devices/simile/service/serviceSimile";

import ColumnFilterPopover from "@/devices/simile/filter/ColumnFilterPopover";
import {
  applySimileFilter,
...
import { applySimileSort, type SimileSortState } from "@/devices/simile/filter/useSimileSort";

import { useUnifiedRentalStatus } from "@/devices/simile/derived/useUnifiedRentalStatus";

import { buildColorBulkPatch } from "@/devices/simile/color/applySimileColor";
import type { SimileSoftColor } from "@/devices/simile/color/ColorPopover";

import { mapGetDeviceNoCI, setHasDeviceNoCI } from "@/lib/deviceNo";

export type SimileGridHandle = {
  reload: (options?: { silent?: boolean }) => Promise<void>;
  applyColorToSelection: (color: SimileSoftColor, mode: "text" | "cell") => Promise<void>;
};
...
type Props = {
  ...
  sortState: SimileSortState;
  onSortStateChange: (next: SimileSortState) => void;
};
...
const SimileGrid = forwardRef<SimileGridHandle, Props>(function SimileGrid(props, ref) {
  ...
  const [columnOrderState, setColumnOrderState] = useState<string[]>(() => [...simileColumns]);
  ...
  const displayRows = useMemo(() => {
    const filtered = applySimileFilter(rows as any, props.filterState);
    const sorted = applySimileSort(filtered as any, props.sortState);
    return sorted as SimileRow[];
  ...
  function getSelectedRowSlice() {
    if (!selectedRowRange) return { start: 0, end: -1, slice: [] as SimileRow[] };
  ...
  async function clearSelection() {
    ...
    if (selectedCellRange) {
      ...
      setRows(local);
      if (updates.length) await bulkPatchSimile({ updates });
      setContextMenu(null);
      return;
    }
  ...
  async function handleInsertRows() {
    ...
    await insertSimileRows({ count, beforeId, afterId });
    await reload();
  ...
  async function applyColorToSelection(color: SimileSoftColor, mode: "text" | "cell") {
