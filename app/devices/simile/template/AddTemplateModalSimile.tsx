...
export default function AddTemplateModalSimile({ open, onClose, onChanged }: Props) {
  ...
  const baseSet = useMemo(() => new Set<string>([...(simileColumns as unknown as string[])]), []);
  ...
  async function loadColumns() {
    ...
    setError("");
    try {
      const r = await fetch("/api/devices/simile/columns", { cache: "no-store" });
  ...
  async function handleAdd() {
    ...
    setError("");
    try {
      const r = await fetch("/api/devices/simile/columns", {
        method: "POST",
  ...
  async function handleDelete(key: string) {
    ...
    setError("");
    try {
      const r = await fetch(`/api/devices/simile/columns/${encodeURIComponent(k)}`, {
        method: "DELETE",
      });
    ...
    >
      ...
      >
        <div className="px-4 py-3 border-b flex items-center justify-between">
          <div className="font-semibold text-slate-800">시밀래 양식(컬럼) 추가/삭제</div>
          <button
          ...
          <div className="border rounded p-3">
            ...
            <div className="flex flex-col gap-2">
              ...
              <select
                ...
                disabled={loading}
              >
                {(order.length ? order : [...(simileColumns as unknown as string[])]).map((k) => (
