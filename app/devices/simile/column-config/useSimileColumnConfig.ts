...
import {
  ...
  simileColumns,
} from "@/devices/simile/columns/simileColumns";
...
function sanitizeWidths(input: any, globalOrder: string[]) {
  ...
  for (const k of simileColumns as unknown as string[]) {
...
export function useSimileColumnConfig() {
  const defaultGlobal = [...(simileColumns as unknown as string[])];
  ...
  async function loadUserConfig(globalOrder: string[]) {
    const r = await fetch("/api/devices/simile/grid-settings", { cache: "no-store" });
    if (!r.ok) return;
  ...
  async function saveNow(cfg: ColumnConfig) {
    await fetch("/api/devices/simile/grid-settings", {
      method: "POST",
