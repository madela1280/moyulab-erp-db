...
export default function AddTemplateModalSwingMaxi({ open, onClose, onChanged }: Props) {
  ...
  const baseSet = useMemo(
    () => new Set<string>([...(swingMaxiColumns as unknown as string[])]),
    []
  );
  ...
  async function handleAdd() {
    ...
    setError("");
    try {
      const r = await fetch("/api/devices/swingMaxi/columns", {
        method: "POST",
  ...
  async function handleDelete(key: string) {
    ...
    setError("");
    try {
      const r = await fetch(`/api/devices/swingMaxi/columns/${encodeURIComponent(k)}`, {
        method: "DELETE",
      });
          ...
          <div className="border rounded p-3">
            ...
            <div className="flex flex-col gap-2">
              ...
              <select
                ...
                disabled={loading}
              >
                {(order.length ? order : [...(swingMaxiColumns as unknown as string[])]).map((k) => (
