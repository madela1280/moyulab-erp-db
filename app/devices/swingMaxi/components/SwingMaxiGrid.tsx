...
import { swingMaxiColumns } from "@/devices/swingMaxi/columns/swingMaxiColumns";
import { useSwingMaxiRows } from "@/devices/swingMaxi/hooks/useSwingMaxiRows";
import {
  bulkDeleteSwingMaxi,
  bulkPatchSwingMaxi,
  insertSwingMaxiRows,
...
import ColumnFilterPopover from "@/devices/swingMaxi/filter/ColumnFilterPopover";
import {
  applySwingMaxiFilter,
...
import { useUnifiedRentalStatus } from "@/devices/swingMaxi/derived/useUnifiedRentalStatus";

import { buildColorBulkPatch } from "@/devices/swingMaxi/color/applySwingMaxiColor";
...
export type SwingMaxiGridHandle = {
...
type Props = {
  ...
  sortState: SwingMaxiSortState;
  onSortStateChange: (next: SwingMaxiSortState) => void;
};
...
const SwingMaxiGrid = forwardRef<SwingMaxiGridHandle, Props>(function SwingMaxiGrid(
  props,
  ref
) {
  const { rows, setRows, setTotalCount, loading, error, reload } = useSwingMaxiRows();
  ...
  const displayRows = useMemo(() => {
    const filtered = applySwingMaxiFilter(rows as any, props.filterState);
    const sorted = applySwingMaxiSort(filtered as any, props.sortState);
    return sorted as SwingMaxiRow[];
  ...
  function getSelectedRowSlice() {
    if (!selectedRowRange) return { start: 0, end: -1, slice: [] as SwingMaxiRow[] };
  ...
  function getActiveSwingMaxiRowId(): number | null {
    try {
  ...
  async function handleFocus(rowId: number, key: string, initialValue: string, e: any) {
    ...
    const pending = existingPending ?? acquireLock("swingMaxi", rowId);
    ...
    if (result?.ok) {
      ...
      await releaseLock("swingMaxi", rowId).catch(() => {});
      return;
    }
  ...
  async function saveCell(rowId: number, key: string, value: string) {
    ...
    await patchSwingMaxiRow(rowId, { [key]: v });
  }

  async function ensureSwingMaxiRowLock(rowId: number) {
    ...
    const retry = await acquireLock("swingMaxi", rowId).catch(() => null);
    if (retry?.ok) {
      myRowLocksRef.current[rowId] = true;
      setMyRowLocks((prev) => ({ ...prev, [rowId]: true }));
      return true;
  ...
  async function clearSingleSelectedCell() {
    ...
    const hasLock = await ensureSwingMaxiRowLock(row.id);
  ...
  function getDisplayValue(row: SwingMaxiRow, colKey: string) {
  ...
  async function clearSelection() {
    ...
    if (selectedCellRange) {
      ...
      setRows(local);
      if (updates.length) await bulkPatchSwingMaxi({ updates });
      setContextMenu(null);
      return;
    }
    ...
    await bulkPatchSwingMaxi({ updates });
    setContextMenu(null);
  }
  ...
  async function handleInsertRows() {
    ...
    await insertSwingMaxiRows({ count, beforeId, afterId });
    await reload();
  ...
  async function applyColorToSelection(color: SwingMaxiSoftColor, mode: "text" | "cell") {
    if (!selectedCellRange) return;
                      ...
                      >
                        <input
                          ...
                          onBlur={async (e) => {
                            ...
                            if (!hasLock) {
                              const retry = await acquireLock("swingMaxi", blurRowId).catch(() => null);
                              if (retry?.ok) {
                                hasLock = true;
                                myRowLocksRef.current[blurRowId] = true;
                                setMyRowLocks((prev) => ({ ...prev, [blurRowId]: true }));
                            ...
                            try {
                              ...
                              const nextFocusedRowId = getActiveSwingMaxiRowId();
                              ...
                              if (!keepRowLock && hasLock) {
                                await releaseLock("swingMaxi", blurRowId).catch(() => {});
