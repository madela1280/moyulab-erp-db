...
import { lactinaColumns } from "@/devices/lactina/columns/lactinaColumns";
import { useLactinaRows } from "@/devices/lactina/hooks/useLactinaRows";
import {
  bulkDeleteLactina,
  bulkPatchLactina,
  insertLactinaRows,
...
import ColumnFilterPopover from "@/devices/lactina/filter/ColumnFilterPopover";
import {
  applyLactinaFilter,
...
import { applyLactinaSort, type LactinaSortState } from "@/devices/lactina/filter/useLactinaSort";

import { useUnifiedRentalStatus } from "@/devices/lactina/derived/useUnifiedRentalStatus";

import { buildColorBulkPatch } from "@/devices/lactina/color/applyLactinaColor";
import type { LactinaSoftColor } from "@/devices/lactina/color/ColorPopover";

export type LactinaGridHandle = {
  reload: (options?: { silent?: boolean }) => Promise<void>;
  applyColorToSelection: (color: LactinaSoftColor, mode: "text" | "cell") => Promise<void>;
};
...
type Props = {
  ...
  sortState: LactinaSortState;
  onSortStateChange: (next: LactinaSortState) => void;
};
...
const LactinaGrid = forwardRef<LactinaGridHandle, Props>(function LactinaGrid(props, ref) {
  const {
    rows,
    setRows,
    setTotalCount,
  ...
  const [columnOrderState, setColumnOrderState] = useState<string[]>(() => [...lactinaColumns]);
  ...
  const displayRows = useMemo(() => {
    const filtered = applyLactinaFilter(rows as any, props.filterState);
    const sorted = applyLactinaSort(filtered as any, props.sortState);
    return sorted as LactinaRow[];
  ...
  function getSelectedRowSlice() {
    if (!selectedRowRange) return { start: 0, end: -1, slice: [] as LactinaRow[] };
 ...
 async function handleFocus(rowId: number, key: string, initialValue: string, e: any) {
  ...
  async function clearSelection() {
    ...
    if (selectedCellRange) {
      ...
      setRows(local);
      if (updates.length) await bulkPatchLactina({ updates });
      setContextMenu(null);
      return;
    }
    ...
    await bulkPatchLactina({ updates });
    setContextMenu(null);
  }
  ...
  async function handleInsertRows() {
    ...
    await insertLactinaRows({ count, beforeId, afterId });
    await reload();
  ...
  // ===== 칼라 적용(선택 영역 기반) =====
  async function applyColorToSelection(color: LactinaSoftColor, mode: "text" | "cell") {
    if (!selectedCellRange) return;
                    ...
                    return (
                      ...
                      <input
                          ...
                          onBlur={async (e) => {
                            ...
                            if (!hasLock) {
                              const retry = await acquireLock("lactina", blurRowId).catch(() => null);
                              if (retry?.ok) {
                                hasLock = true;
                                myRowLocksRef.current[blurRowId] = true;
                                setMyRowLocks((prev) => ({ ...prev, [blurRowId]: true }));
                            ...
                            try {
                              ...
                              if (!keepRowLock && hasLock) {
                                await releaseLock("lactina", blurRowId).catch(() => {});
