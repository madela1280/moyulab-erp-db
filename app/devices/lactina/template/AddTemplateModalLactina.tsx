...
export default function AddTemplateModalLactina({ open, onClose, onChanged }: Props) {
  ...
  const baseSet = useMemo(() => new Set<string>([...(lactinaColumns as unknown as string[])]), []);
  ...
  async function loadColumns() {
    ...
    setError("");
    try {
      const r = await fetch("/api/devices/lactina/columns", { cache: "no-store" });
      const j = await r.json().catch(() => ({}));
      const next = Array.isArray(j?.order) ? j.order.map(String) : [];
      setOrder(next);
      if (!referenceKey) {
        // 기본 referenceKey는 "제품명"(있으면) 아니면 첫 컬럼
        const first = next[0] ?? "";
  ...
  async function handleAdd() {
    ...
    setError("");
    try {
      const r = await fetch("/api/devices/lactina/columns", {
        method: "POST",
  ...
  async function handleDelete(key: string) {
    ...
    setError("");
    try {
      const r = await fetch(`/api/devices/lactina/columns/${encodeURIComponent(k)}`, {
        method: "DELETE",
      });
  ...
  return (
    ...
    >
      <div className="bg-white w-[720px] max-w-[95vw] rounded shadow border" onMouseDown={(e) => e.stopPropagation()}>
        <div className="px-4 py-3 border-b flex items-center justify-between">
          <div className="font-semibold text-slate-800">락티나 양식(컬럼) 추가/삭제</div>
          ...
          <div className="border rounded p-3">
            ...
            <div className="flex flex-col gap-2">
              ...
              <select
                ...
                disabled={loading}
              >
                {(order.length ? order : [...(lactinaColumns as unknown as string[])]).map((k) => (
