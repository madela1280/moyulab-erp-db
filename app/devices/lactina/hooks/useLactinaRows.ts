...
import type { LactinaRow } from "@/devices/lactina/service/serviceLactina";
import { listLactinaRows } from "@/devices/lactina/service/serviceLactina";
...
/**
 * 락티나 rows 로딩/리프레시 상태 훅
 * - 실제 데이터는 /api/devices/lactina 에서만
 * - 그리드/뷰에서는 이 훅이 주는 rows/reload만 사용
 */
export function useLactinaRows() {
  const [rows, setRows] = useState<LactinaRow[]>([]);
