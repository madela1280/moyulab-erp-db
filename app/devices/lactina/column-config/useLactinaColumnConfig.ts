...
import {
  ...
  lactinaColumns,
} from "@/devices/lactina/columns/lactinaColumns";
...
function sanitizeWidths(input: any, globalOrder: string[]) {
  ...
  // 기본컬럼은 기본값(20) 세팅
  for (const k of lactinaColumns as unknown as string[]) {
...
export function useLactinaColumnConfig() {
  const defaultGlobal = [...(lactinaColumns as unknown as string[])];
  ...
  // ✅ 전역 컬럼(기본+커스텀) 목록 로드: /api/devices/lactina/columns
  ...
  // ✅ 유저별 컬럼 설정 로드: /api/devices/lactina/grid-settings
  async function loadUserConfig(globalOrder: string[]) {
    const r = await fetch("/api/devices/lactina/grid-settings", { cache: "no-store" });
    if (!r.ok) return;
  ...
  async function saveNow(cfg: ColumnConfig) {
    await fetch("/api/devices/lactina/grid-settings", {
      method: "POST",
